...
namespace cocos2d 
{
	...
	void CCScrollLayer::visit()
	{
		...
		if (m_bShowPagesIndicator)
		{
			...
			CGFloat d = 22.0f; //< Distance between points.
			...
			glPointSize(16.0f * CC_CONTENT_SCALE_FACTOR());

			// Draw Gray Points
			glColor4ub(0x96,0x96,0x96,0x77);
	...
	{	
		...
		CCAction* changePage = 
			CCSequence::actionOneTwo(
					CCEaseElasticOut::actionWithAction(CCMoveTo::actionWithDuration(0.5f, positionForPageWithNumber(pageNumber)), 1.0),
